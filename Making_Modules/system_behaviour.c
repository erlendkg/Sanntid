...
int network_elevator_mode(Elev_info *this_elevator, int server_socket, char const *server_ip) {
  ...
  while(1) {
    if(this_elevator->is_connected_to_network == 1) {
      ...
      if(this_elevator->current_floor != return_current_floor() && return_current_floor() != -1){
        memset(message, 0, sizeof(message));

        this_elevator->current_floor = return_current_floor();

        printf("Elev is now on floor: %d, going to: %d\n", this_elevator->current_floor, this_elevator->desired_floor[0] );
        printf("Elev num is: %d\n", this_elevator->num );

        sprintf(message, "<1E%dF%d>", this_elevator->num, this_elevator->current_floor);

        send(this_elevator->server_socket, message, sizeof(message), 0);

        // send_all(this_elevator->server_socket, message, sizeof(message));
  ...
  while(1) {
    if (recv(my_this_elevator->server_socket, buffer, sizeof(buffer), 0) == 0){
      ...
      //Stop thread
            ...
            printf("***********************\n");
            printf("Melding fra Master: %s\n", buffer);
            printf("***********************\n\n");

            unpack_message_to_variables(buffer, &a, &b, &c, &my_this_elevator->desired_floor[0]);
            printf("desired_floor is now: %d\n", my_this_elevator->desired_floor[0]);
            memset(buffer, 0, sizeof(buffer));

    }
  }
}
//Gi disse to mer beskrivende navn
